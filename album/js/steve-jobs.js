...
function loadPage(page, obj, m) {
	$.ajax({
		type: 'post',
		url: 'pages/page' + m + '.html'
		
		
		//type: 'get',
		// url: '/index20190107/yin/pages/page' + m + '.html'
	}).done(function(pageHtml) {
		var $book = $('.sj-book .p' + page);
		$book.html(pageHtml);
		if(m === 1){
			$book.find('.book-img img').attr('src', obj.img)
		}else{
			var html = "";
			var n = obj.start;
			for(var i = 0; i < obj.img.length; i++){
				html += '<a href="#page/'+ n +'" class="box"><img src="'+obj.img[i] +'" alt=""></a>';
				n++;
			}
			$book.find('.book-catalog .list').html(html)
		}
	});

}

function addPage(page, book, img) {
	var id, pages = book.turn('pages');
	if (!book.turn('hasPage', page)) {
		var mc = 'own-size m' + img.id
		var element = $('<div />',
			{'class': mc,
